function [RMSE_A, AGFI, ARIQ, ARIQ1, ARIJ, ARIJ1] = UFA_example(J,Q,eSx,flc,Rndst)

% Running example
%[RMSE_A, AGFI, ARIQ, ARIQ1, ARIJ, ARIJ1] = UFA_example(30,4,0.01,0,50);

% INPUT:
%
% J     Number of variables
% Q     Number of classes of variables (Q<<J)
% eSx   Level of error for the correlation matrix
% flc   0 if a factor can have a single MV, 1 otherwise
% Rndst Number of random starts

% OUTPUT:
%
% RMSE_A   RMSE between the generated and the estimated A
% AGFI     Adjusted Goodness of Fit Index
% ARIQ     Adjusted Rand Index for the first Q factors 
% ARIQ1    Mean ARIQ equal to 1    
% ARIJ     Adjusted Rand Index for the first the 2Q-1 factors 
% ARIJ1    Mean ARIJ equal to 1 

% Data generation
[Rx,tSx,A,dPsi,VQ,V,C] = RandGenSx(J, Q, eSx, flc);
% Apply the model
[Jtrufa, Aufa, Psiufa, Yufa, fufa, inufa, AGFI] = UFA(tSx, Q, 'Rndst',Rndst);
% Evaluation metrics
RMSE_A = sum(sum((Aufa*Aufa'-A*A').^2))/(J*Q);
ARIQ = mrand(Jtrufa(:,1:Q)'*VQ);
% ARI for the remaining Q-1 levels 
arim = [];
for q =1:(Q-1)
    arim(q) = rand_index(Jtrufa(:,Q+q),V(:,Q+q));
end
ARIJ = (sum(arim) + ARIQ)/Q; %ARI for the J-tree
